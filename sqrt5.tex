...
an elliptic curve $E$ over $F$ we get two two associated embeddings into
...
\begin{eqnarray*}
\Omegapp_E &=& \Omegap_E\Omegap_{\Ebar} \\
\Omegapm_E &=& \Omegap_E\Omegam_{\Ebar} \\
\Omegamp_E &=& \Omegam_E\Omegap_{\Ebar} \\
\Omegamm_E &=& \Omegam_E\Omegam_{\Ebar}.
\end{eqnarray*}
...
\begin{eqnarray*}
    ...
    \tau_1(\Ebar) &=& \frac{\Omegapm_E}{\Omegapp_E} = \frac{\Omegam_E}{\Omegap_E} \\
    \tau_2(\Ebar) &=& \frac{1}{2}\left(1 + \frac{\Omegapm_E}{\Omegapp_E}\right) = \frac{1}{2}\left(1 + \frac{\Omegam_{\Ebar}}{\Omegap_{\Ebar}}\right)
\end{eqnarray*}
...
\[
    \alpha = \frac{\sigma_1(c_4) + \sigma_2(c_4)}{2}
\]
and
\[
...
compute rational integers $\alpha$ and $\beta$ and an integer
...
$E$. Techniques for getting around precision issues were somewhat discussed already, so
we will mostly ignore them here. Now, to proceed, we construct four lists of characters
up to some conductor bound $M$ (we are restricting to prime modulus here for simplicity,
as primitivity is ensure, but this is not necessary):
\[
...
\]
These numbers should now all be integer multiple of the mixed periods, so to get an idea
...
\[
\begin{split}
\mathcal{L}^{--} &= \{-33.5784397861927, -3.73093775700518, -18.6546887333924 \} \\ 
\mathcal{L}^{-+} &= \{18.2648617736017i, 32.8767511924049i, 3.65297284727497i \} \\
\mathcal{L}^{+-} &= \{41.4805656925343i, 8.29611313844135i, 41.4804252145628i \} \\
\mathcal{L}^{++} &= \{32.4909940826153, 162.454969943926, 162.455140176447 \}.
\end{split}
\]
...
\[
\begin{split}
       \{1.000, 8.99999999280237, 1.80000000354261\}  &\approx \{1, 9, 9/5\}\\
       \{1.000, 0.555555555556876, 4.99999932581674\} &\approx \{1, 5/9, 5\}\\
       \{1.000, 5.00000000003948, 1.00000338660876\}  &\approx \{1, 5, 1\} \\ 
       \{1.000, 0.200000000577576, 0.199999791002771\} &\approx \{1, 1/5, 1/5\}
\end{split}
\]
which may give an indication of the accuracy of our values.
.. .. .. 
...
We can compute all possible j-invariants for which an elliptic curve over $F$ with have CM.  
For curves of norm conductor up through 199, 
we find 8 curves with CM, all with the same conductor which has norm conductor 81.

\begin{center}
\begin{table}[h]
\caption{Curves over $\Q(\sqrt{5})$ with CM  and norm conductor up to 199\label{table:counts}}
\begin{tabular}{|c|l|l|}\hline
Norm & a-invariants & j-invariant \\\hline
 $81$ & $\left[1, -1, a, -2a, a\right]$ & $-85995a - 52515$ \\\hline
 $81$ & $\left[1, -1, a + 1, a - 2, -2a + 1\right]$ & $-85995a - 52515$ \\\hline
$81$ & $\left[a, -a - 1, a, -128a - 80, 921*a + 569\right]$ & $16554983445a + 10231546590$ \\\hline
$81$ & $\left[a + 1, 1, 0, 129a - 207, -793a + 1283\right]$ & $16554983445a + 10231546590$ \\\hline
$81$ & $\left[a, -a - 1, a + 1, -1153a - 719, -21854a - 13501\right]$ & $16554983445a + 10231546590$ \\\hline
$81$ & $\left[a + 1, 1, 1, 1153a - 1871, 23006a - 37225\right]$ & $16554983445a + 10231546590$ \\\hline
$81$ & $\left[1, -1, a, 13a - 15, 20a - 26\right]$ & $-85995a - 52515$ \\\hline
$81$ & $\left[1, -1, a + 1, -14a - 2, -21a - 6\right]$ & $-85995a - 52515$ \\\hline
\end{tabular}
\end{table}
\end{center}
