...
of the known traces of Frobius to dramatically reduce our search
space. The concept is fairly elementary: Suppose we have a newform
$f$, with Hecke eigenvalue $a_\p$, then we know the point count of
$E_f/(\cO_F/\p)$. Now rather than enumerating over all curves, we
simply search over lifts of the curves over $\cO_F/\p$ with the
correct point count.

It is possible to use multiple Hecke eigenvalues
$a_{\p_1},\ldots,a_{\p_k}$ by lifting curves with correct point counts
in the $\cO_F/\p_i$'s to curves in $\cO_F/\prod \p_i$ before lifting
into $F$. Like wheel factorization, this reduces the overall search
space, but also rapidly increases the number of curves to lift.

While this method will eventually terminate just like naive enumeration, it
suffers from the same issue of potentially being very slow due to large
coefficients. However, for many curves this is not the case, and because of
its simplicity, an implementation can be quite efficient. With that in
mind, this can be a very useful technique when we are building tables of
elliptic curves based upon newforms and we would like to reduce the number
of newforms that we must run through slow techniques.
