...
space. Suppose we have a newform $f$, with Hecke eigenvalues $a_\p$,
so we know $\#E_f(R/\p)$. Instead of enumerating over all curves as in
Section~\ref{sec:naive}, we search over lifts of the curves
over $R/\p$ with the given point counts.
...
$F$. This reduces the overall search space, but also rapidly increases
...
slow due to large coefficients. However, many curves have relatively
small coefficients, and because of its simplicity, an implementation
can be quite efficient.  Thus this is a useful technique when we are
building tables of elliptic curves associated to newforms and would
like to reduce the number of newforms that we must run through slower
techniques.
